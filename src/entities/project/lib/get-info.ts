...
export function getDeploymentStatusColor(status?: Deployment['status']) {
  switch (status) {
    case 'deployed':
      return 'green';
    case 'failed':
...
export function getEnvironmentStatusColor(status?: Environment['status']) {
  switch (status) {
    case 'healthy':
      return 'green';
    case 'degraded':
...
export function getHealthDescription(health: ProjectHealth) {
  switch (health) {
    case 'healthy':
      return 'Project is stable. No active issues detected.';
    case 'running':
      return 'Pipeline or deployment is currently running.';
    case 'warning':
      return 'Some signals require attention.';
    case 'critical':
      return 'Project has blocking CI/CD issues.';
    default:
      return 'Project status is unknown.';
  }
}
